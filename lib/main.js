...
var aeq = function select(selector) {
	'use strict';
		...
		while (parts.length > 0)
		{
			...
			switch (part.type)
			{
				case 'comp':
					...
					results = comps = aeq.getCompositions().filter(function (c) {
				...
				case 'layer':
					...
					results = layers = aeq.getLayers(comps).filter(function (l) {
				...
				case 'prop':
					...
					results = props = aeq.getProperties(layers, { separate : true }).filter(function (p) {
