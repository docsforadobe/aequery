...
aeq.extend({
	 ...
	 * Writes a file.
	 *
	 * @param      {File|string}  filePath   	Path of file to write to
	 * @param      {string}  contents   			Data to write to the file
	 * @param      {string}  [encoding=UTF-8] Encoding method
	 * @param      {bool}  overwrite  				True if wanting to overwrite the file
	 * @return     {File|null}  							New file
	 */
	writeFile: function(filePath, contents, encoding, overwrite) {
		var file = aeq.getFileObject(filePath);
		encoding = aeq.setDefault(encoding, "UTF-8");
	 ...
	 *
	 * @param      {Folder|string}  folder  Path to folder
	 * @return     {Folder|null}  					New folder
	 */
	checkFolder: function(folder) {
		return aeq.getFolderObject(folder).exists;
	},
