...
aeq.renderqueue = {
	...
	ensureRenderPathExists: function(outputModule) {
		var outputPath = aeq.renderqueue.getSettings(outputModule)["Output File Info"];
		aeq.file.ensureFolderExists(outputPath["Base Path"] + aeq.file.pathSeparatorSymbol + outputPath["Subfolder Path"]);
	},

	omTemplateExists: function(templateName) {
		var tempComp = aeq.comp.create();
		var tempRQItem = aeq.renderqueue.queue(tempComp);
		var itemOM = tempRQItem.outputModule(1);

		var templateExists = aeq.exists(itemOM.templates, function(template) {
	...
	rqTemplateExists: function(templateName) {
		...
		var templateExists = aeq.exists(tempRQItem.templates, function(template) {
			return template == templateName;
		});
