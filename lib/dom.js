...
aeq.extend({
	...
	getLayers: function(comps) {
		...
		for (var c=0; c < comps.length; c++) {
			var comp = comps[c];
			var layers = Array.prototype.slice.call(comp.layers, 1);
			var len = comp.layers.length;
			// Because the last layer is at index LayerCollection.length and slice only goes up to
			// length - 1, we have to push the last layer manually
			layers.push(comp.layers[len]);

			arr = arr.concat( layers );
		}
	...
	getChildren: function(obj) {
		var ret, len;
		if (aeq.isComp(obj)) {
			ret = Array.prototype.slice.call(obj.layers, 1);
			len = obj.layers.length;
			// Because the last layer is at index LayerCollection.length and slice only goes up to
			// length - 1, we have to push the last layer to the return value
			ret.push(obj.layers[len]);
			return ret;
		}
