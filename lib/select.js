	...
	while (parts.length > 0)
	{
		...
		switch (part.type)
		{
			...
			case 'comp':
				...
				results = comps =  aeq.getCompositions().filter(filter);
				break;

			case 'layer':
				aeq.assertIsNull(layers, 'Illegal token position for "layer"');
				results = layers = aeq.getLayers(comps).filter(filter);
				break;

			case 'property':
			case 'prop':
				aeq.assertIsNull(props, 'Illegal token position for "' + part.type + '"');
				results = props = aeq.getProperties(layers, { separate : false }).filter(filter);
				break;

			case 'effect':
				results = effects = aeq.getEffects(layers).filter(filter);
				break;
